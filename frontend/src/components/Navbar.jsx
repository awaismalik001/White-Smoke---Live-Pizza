...
export default function Navbar() {
  ...
  const isLinkActive = (link) => {
    ...
    if (link.type === 'home') {
      return isHome && (activeSection === 'home' || !activeSection)
    }
    if (link.type === 'section') {
    ...
    <>
      <header className="fixed top-0 left-0 right-0 z-50 flex justify-center px-4 py-4 sm:py-5 transition-all duration-500 pointer-events-none">
        ...
        >
          ...
          >
            <div className="w-8 h-8 rounded-full bg-brand-red flex items-center justify-center text-white font-heading font-extrabold text-sm shadow-[0_0_15px_rgba(229,0,0,0.7)] group-hover:scale-105 transition-transform">
              W
            </div>
            <span className="font-heading font-extrabold text-base tracking-wide text-white">
                ...
                return (
                  <button
                    ...
                    className={`py-2 text-left text-sm font-semibold tracking-wide border-b border-white/5 transition-colors ${
                      active ? 'text-brand-red' : 'text-zinc-300'
                    }`}
                  >
                    {link.name}
                  </button>
